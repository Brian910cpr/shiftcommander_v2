...
class LiveStateStoreSmokeTests(unittest.TestCase):
    def setUp(self):
        ...
        # A missed calendar mock must fail the test even if the app catches the
        # connection error and falls back to cached data.
        ...
        self.enterContext(patch.object(self.server, "GOOGLE_CALENDAR_JUNE_MIRROR_FILE", str(self.state_dir / "calendar_mirror.json")))
        self.calendar = self.enterContext(patch.object(
            self.server, "load_google_calendar_june_mirror_payload",
            return_value={"build": {"feed_status": "fixture"}, "shifts": []},
        ))
    ...
    def test_schedule_reads_use_mocked_calendar_without_creating_mirror(self):
        response = self.client.get("/api/bootstrap")
        self.assertEqual(response.status_code, 200)
        self.calendar.assert_called()
        self.assertFalse((self.state_dir / "calendar_mirror.json").exists())
