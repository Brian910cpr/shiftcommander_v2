...
* preserve_existing_assignments() behavior
* lock handling logic
...
## CyberPC-First Development Work

Use CyberPC/local tools first for routine deterministic development work, including:

* repository scans and searches
* builds and syntax checks
* automated tests
* fixture and artifact generation
* bulk or mechanical edits
* resolver validation and reproducibility checks

Reserve ChatGPT/Codex reasoning capacity for work that materially benefits from it, including:

* diagnosis of unclear failures
* scheduling-policy interpretation
* architecture and cross-system design
* ambiguous implementation decisions
* operational tradeoffs that require judgment

Keep automated work bounded. Prefer local deterministic execution when it can answer the question reliably, and avoid spending Codex reasoning on repetitive work that CyberPC can perform directly. Protect ChatGPT/Codex capacity for operational use and higher-value reasoning.

When a repetitive deterministic task appears more than once, prefer creating or reusing a local worker or script rather than repeating AI-driven execution. Temporary one-off workers are acceptable for bounded bulk tasks when they produce inspectable outputs and do not silently change scheduling policy.
