...
export function buildPlannedSchedule({
  ...
  for (const u of (units||[])){
    ...
    for (const seat of unitSeats){
      ...
      if (eligible === 0) issues.push(`ZERO eligible: ${u.name} → ${seat.label} (req: ${req.join(", ")||"none"})`);
      else if (eligible < 3) issues.push(`Low candidates (${eligible}): ${u.name} → ${seat.label}`);
    }
  }
  if (issues.length){
    ...
    for (const shift of shifts){
      ...
      for (const u of unitsSorted){
        ...
        for (const seat of seats){
          ...
          addLog("info", `┌─ ${dateIso} ${shift} :: ${u.name} → ${seat.label} (req: ${req.join(", ")||"none"})`);
          ...
          if (pick){
            ...
            addLog("ok", `└─ ✅ ${targetKey.toUpperCase()}: ${pick.name}`);
          } else {
            addLog("warn", `└─ ⚠️ ${targetKey.toUpperCase()}: OPEN`);
          }
