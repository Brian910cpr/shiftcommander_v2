...
# ---- Table names (v2 namespace to avoid collisions with existing schema) ----
T_WEEKS = "sc_weeks"
T_SHIFTS = "sc_shifts"
T_SHIFT_CONFIG = "sc_shift_config"
...
def ensure_tables(conn: sqlite3.Connection) -> None:
    ...
    CREATE TABLE IF NOT EXISTS {T_WEEKS} (
        ...
        start_date TEXT NOT NULL,   -- YYYY-MM-DD (Thursday)
        end_date   TEXT NOT NULL,   -- YYYY-MM-DD (Wednesday)
    ...
    CREATE TABLE IF NOT EXISTS {T_SHIFTS} (
        ...
        FOREIGN KEY(week_id) REFERENCES {T_WEEKS}(week_id) ON DELETE CASCADE
    );
    """)

    conn.execute(f"""
    CREATE TABLE IF NOT EXISTS {T_SHIFT_CONFIG} (
        shift_id TEXT PRIMARY KEY,
        first_out_override_unit_id TEXT NULL,
        staffed_unit_id TEXT NULL,
        is_salary_only INTEGER NOT NULL DEFAULT 0,
        active INTEGER NOT NULL DEFAULT 1,
        FOREIGN KEY(shift_id) REFERENCES {T_SHIFTS}(shift_id) ON DELETE CASCADE
    );
    """)
    ...
    # helpful indexes
    conn.execute(f"CREATE INDEX IF NOT EXISTS idx_{T_SHIFTS}_week ON {T_SHIFTS}(week_id);")
    conn.execute(f"CREATE INDEX IF NOT EXISTS idx_{T_SEATS}_shift ON {T_SEATS}(shift_id);")
    conn.execute(f"CREATE INDEX IF NOT EXISTS idx_{T_SEATS}_unit_seat ON {T_SEATS}(unit_id, seat_id);")

def units_list(conn: sqlite3.Connection):
    # Prefer your seeded units table if present
    try:
...
def ensure_week_not_exists(conn: sqlite3.Connection, start_date: str):
    row = conn.execute(f"SELECT week_id FROM {T_WEEKS} WHERE start_date = ?", (start_date,)).fetchone()
    if row:
        raise SystemExit(f"ERROR: Week already exists for start_date={start_date} (week_id={row[0]}).")
...
def create_week(conn: sqlite3.Connection, start_date: date, first_out_default: str, lock_rule_days: int = 29):
    ...
    # 14 shifts: DAY 06-18, NIGHT 18-06
    for day_index in range(7):
        ...
        for slot, s_start, s_end in [
            ...
            conn.execute(
                f"INSERT INTO {T_SHIFT_CONFIG}(shift_id, staffed_unit_id, active) VALUES (?,?,1)",
                (shift_id, first_out_default)
            )
            ...
            for unit_id in units:
                ...
                for seat_id in ("ATTENDANT","DRIVER"):
                    conn.execute(
                        f"INSERT INTO {T_SEATS}(seat_record_id, shift_id, unit_id, seat_id, layer) VALUES (?,?,?,?,?)",
...
def main():
    ap = argparse.ArgumentParser(description="Create a Thu->Wed schedule week (v2 tables, avoids collisions).")
    ap.add_argument("--db", required=True, help="Path to shiftcommander.db")
    ap.add_argument("--start", required=True, help="Week start date (Thursday) in YYYY-MM-DD")
    ap.add_argument("--first-out", default="AMB121", help="Default first-out unit for the week (AMB120/AMB121/AMB131)")
    args = ap.parse_args()
